  ...
  if (loading) {
    return (
      <div className="rounded-xl border border-white/6 bg-surface p-6">
        <p className="text-sm text-dim">{t("garage.report.loading")}</p>
      </div>
    );
  }
  ...
  return (
    <div className="animate-panel-in rounded-xl border border-white/6 bg-surface overflow-hidden">
      ...
      <div className="flex items-start justify-between gap-4 border-b border-white/6 px-5 py-4">
        <div className="min-w-0 space-y-1">
          <div className="flex flex-wrap items-center gap-2">
            <h2 className="text-sm font-semibold text-hi">
              {check.title ?? check.listingUrl ?? t("garage.report.fallbackTitle")}
            </h2>
        ...
        <button
          onClick={onClose}
          aria-label={t("garage.report.collapseAriaLabel")}
          className="shrink-0 flex items-center gap-1 text-xs text-dim transition-colors hover:text-mid"
        >
          <ChevronUp className="h-4 w-4" />
          <span className="hidden sm:inline">{t("garage.report.collapse")}</span>
        </button>
      </div>
      ...
      <div className="p-5 space-y-5">
        {check.status === "Pending" || check.status === "Processing" ? (
          <div className="flex items-center gap-3">
            <StatusBadge status={check.status} />
            <span className="text-sm text-mid">— {t("garage.report.analysisInProgress")}</span>
          </div>
        ) : check.status === "Failed" ? (
          <div className="space-y-1.5">
            <p className="text-sm font-medium text-bad">{t("garage.report.analysisFailed")}</p>
            {check.failureReason && (
